...
player_images = {
    "stop": pygame.image.load("images\player\stop.png"),
    "goL1": pygame.image.load("images\player\goL1.png"),
    "goL2": pygame.image.load("images\player\goL2.png"),
    "goR1": pygame.image.load("images\player\goR1.png"),
    "goR2": pygame.image.load("images\player\goR2.png"),
    "wR": pygame.image.load("images\player\wR.png"),
    "wL": pygame.image.load("images\player\wL.png"),
}
weapons = {
    "m1L": pygame.image.load("images\weapon\m1L.png"),
    "m1R": pygame.image.load("images\weapon\m1R.png"),
}

profile_photo = os.listdir("images\profile")
if profile_photo:
    profile_photo = pygame.image.load(
        "images\profile\\" + os.listdir("images\profile")[0]
    )
else:
...
class Menu:
    def __init__(self):
        self.buttons = [
            ...
            [
                (SIZE[0] / 2 - 150, SIZE[1] - 250, 300, 100),
                (255, 153, 51),
                (SIZE[0] + 30, SIZE[1] - 55),
                "Маркет",
                32,
            ...
            [
                (SIZE[0] / 2 - 150, SIZE[1] - 370, 300, 100),
                (255, 153, 51),
                (SIZE[0] + 30, SIZE[1] - 55),
                "Инвентарь",
                32,
        ...
        self.buttons_ready = [
            Button(but[0], but[1], but[3], but[4], width_text=but[5])
    ...
    def start(self):
        ...
        reg_button = Button(
            (20, 240, 200, 80), (255, 153, 51), "Сохранить", 24, width_text=25
        )

        tim = time.time()
        player = Player("Vadek", (SIZE[0] / 2 - 32, SIZE[1] / 2 - 32), self.put_on)
        ...
        while True:
            ...
            for button in self.buttons_ready:
                if button.name_button("Играть") and button.press(
                    position_mouse, press_mouse
                ):
                    self.enter_name = False
                ...
                elif button.name_button("Настройки") and button.press(
                    position_mouse, press_mouse
                ):
                    (
                        self.count_players,
                        ...
                        self.ip,
                        self.port,
                    ) = config_game.start(window)
                elif button.name_button("Выйти", "Нее...") and button.press(
                    position_mouse, press_mouse
                ):
                    pygame.quit()
                    exit()
            ...
            if self.buttons_ready[1].cursor(position_mouse):
                ...
                self.buttons_ready[1].replace_text("Выйти")
            ...
            if reg_button.press(position_mouse, press_mouse):
                self.data = self.reg.start(
                    window, self.ip, self.port, self.name, self.moneys, self.coins
                )
                tim = time.time()

            # -------------------------

            player.render(window, player_images)
            # ----------TOP------------
            top.render(players)
            ...
            if (
                self.buttons_ready[3].press(position_mouse, press_mouse)
                and time.time() - tim > 0.3
            ):
                self.inventory, self.moneys = self.market.start()
                tim = time.time()

            if (
                self.buttons_ready[4].press(position_mouse, press_mouse)
                and time.time() - tim > 0.3
            ):
...
class Game:
    ...
    def start(
        self, name, put_on, count_players, ip, port
    ):  # , players={'Muhammed': {self.player.x}}):
        ...
        stolbs = [
            pygame.rect.Rect(1300, 190, 40, 100),
            pygame.rect.Rect(1300, 290, 40, 100),
        ]
        player_rect = pygame.rect.Rect(self.player.x + 20, self.player.y, 50, 100)
        players_rect = [
            [pygame.rect.Rect(0, 0, 50, 100), "name"] for i in range(int(count_players))
        ]
        timer = 0
        bullets = 50
        bullets_fly = []
        while True:
            ...
            mes_from_server = client.get_mes().decode("utf-8").strip()
            if type(mes_from_server) == int:
                if mes_from_server == 0:
                    break
            try:
            ...
            # self.player.render(window, player_images)
            old_cord = [self.player.x, self.player.y]
            bullets_fly = self.player.move(
                keys,
                press_mouse,
                mouse_pos,
                window,
                ...
                if iteration >= 5:  # and user["Name"]!=self.player.name:
                    ...
                    for player in players:
                        if player.name == user["Name"]:
                            player.x, player.y = user["cord"]
                            player.inventory = user["inventory"]
                            player.hp = user["hp"]
                            player.image = user["pos"]

                            player.cursor(mouse_pos, window)
                            player.render(window, player_images)

                        # player2.move()
                    # player2.cursor(mouse_pos, window)
                    # player2.auto_move()#, mouse_pos, window, weapons)'''
