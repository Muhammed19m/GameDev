...
class Player:
    ...
    def render(self, window, player_images, weapons):
    ...
    def move(self, keys, press_mouse, pos, window, weapons, SIZE_MAP, player_rect, block_rect, stolbs_rect, bull,ti, players_rect):
        speed = 3
        t = 1
        ...
        players_hwo_bul_push = ""
        for bul in range(len(self.bullet)):
            for name, player_rect in players_rect.items():

                if self.bullet[bul][0].colliderect(player_rect[1]):
                    players_hwo_bul_push=name
            ...
            if self.bullet[bul][0].collidelistall(block_rect) or self.bullet[bul][0].collidelistall(stolbs_rect):
                ...
                if self.bullet[bul][1]=='r': self.bullet[bul][0].x+=8
                else: self.bullet[bul][0].x-=8
                window.blit(bull, self.bullet[bul][0])
        for ind in self.del_ind:
            self.bullet.pop(ind)
        self.del_ind.clear()
        return [[i[0].x, i[0].y] for i in self.bullet], players_hwo_bul_push
