...
class Player:
    ...
    def move(
        ...
        if self.jump:
            ...
            if player_rect.collidelistall(block_rect) or player_rect.collidelistall(
                stolbs_rect
            ):
                self.y += self.jump_velocity
                self.jump = False
        ...
        if (
            self.y < SIZE_MAP[1] - 100 - 40 and not self.jump
        ):  # and not player_rect.collidelistall(stolbs_rect) and not player_rect.collidelistall(block_rect):
            self.y += 4
            player_rect.x, player_rect.y = self.x + 20, self.y

            if player_rect.collidelistall(stolbs_rect) or player_rect.collidelistall(
                block_rect
            ):
                player_rect.x, player_rect.y = self.x + 20, self.y
                self.y -= 4
        ...
        if press_mouse[2]:
            if pos[0] > self.x:
                ...
                if press_mouse[0] and ti % 40 == 0:
                    self.bullet.append(
                        (pygame.rect.Rect(self.x + 30, self.y + 20, 10, 10), "r")
                    )
            else:
                self.image = "wL"
                ...
                if press_mouse[0] and ti % 40 == 0:
                    self.bullet.append(
                        (pygame.rect.Rect(self.x, self.y + 20, 10, 10), "l")
                    )
        for bul in range(len(self.bullet)):
            if self.bullet[bul][0].x > SIZE_MAP[0] or self.bullet[bul][0].x < 0:
                self.del_ind.append(bul)
            if self.bullet[bul][0].collidelistall(block_rect) or self.bullet[bul][
                0
            ].collidelistall(stolbs_rect):
                self.del_ind.append(bul)
            else:
                if self.bullet[bul][1] == "r":
                    self.bullet[bul][0].x += 4
                else:
                    self.bullet[bul][0].x -= 4
    ...
    def render_name(self, window):
        window.blit(
            Player.font.render(self.name, 1, (0, 153, 51)), (self.x + 15, self.y - 25)
        )
